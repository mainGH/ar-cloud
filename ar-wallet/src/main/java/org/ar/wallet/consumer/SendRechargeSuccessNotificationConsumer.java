...
public class SendRechargeSuccessNotificationConsumer {
    ...
    @RabbitListener(queues = RabbitMqConstants.AR_WALLET_TRADE_COLLECT_QUEUE_NAME, concurrency = "5-10")
    public void onTimeoutTask(Message message, Channel channel) {
        ...
        try {
            ...
            if (asyncNotifyService.sendRechargeSuccessCallback(taskInfo.getOrderNo(), "1")) {
                //处理成功 手动确认消息
