 ...
 */
@Service
public class MemberMatchTimeoutConsumer {
    ...
    public MemberMatchTimeoutConsumer(
            HandleOrderTimeoutService handleOrderTimeoutService
    ) {
        this.handleOrderTimeoutService = handleOrderTimeoutService;
    }

    @RabbitListener(queues = RabbitMqConstants.AR_WALLET_MEMBER_MATCH_DEAD_LETTER_QUEUE, concurrency = "5-10")
    public void onTimeoutTask(Message message, Channel channel) {
        ...
        try {
            //钱包用户卖出匹配超时
            logger.info("Rabbit 消费延时队列消息: 钱包用户卖出匹配超时: {}", taskInfo.getOrderNo());

            if (handleOrderTimeoutService.handleWalletUserSaleMatchTimeout(taskInfo.getOrderNo(), taskInfo.getLastUpdateTimestamp())) {
                //处理成功 手动确认消息
                channel.basicAck(message.getMessageProperties().getDeliveryTag(), false);
                logger.info("钱包用户卖出匹配超时, 消息消费成功: {}", taskInfo.getOrderNo());
            } else {
                logger.error("钱包用户卖出匹配超时, 消息消费失败: {}", taskInfo.getOrderNo());
                //处理失败 抛出异常 进行重试消费
